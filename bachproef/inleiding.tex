...
Het doel van deze bachelorproef is om vanuit deze probleemstelling een praktijkgericht onderzoek uit te voeren dat zowel inzicht oplevert in de prestaties van verschillende LLM’s als concrete handvatten biedt voor studenten. Enerzijds wordt geprobeerd om via gestructureerde experimenten de kwaliteit van LLM gegenereerde Python code te evalueren op het vlak van syntax, stijl, beveiliging, uitvoerbaarheid, performantie, onderhoudbaarheid en functionele correctheid in realistische data engineering taken.
...
\textit{In welke mate verschillen zes open source LLM's (DeepSeek Coder V2, Gemma 4, Llama 3, Mistral, Phi 4 Mini en Qwen 2.5 Coder) in hun vermogen om via Ollama technisch correcte, veilige en onderhoudbare Python code te genereren voor 35 representatieve data analyse en ETL taken, wanneer deze volledig geautomatiseerd wordt geëvalueerd op acht evaluatiedimensies (syntax, stijl, beveiliging, uitvoerbaarheid, performantie, schaalbaarheid, onderhoudbaarheid en functionele correctheid)?}

Deze vraag wordt verder uitgewerkt door per model en per taak na te gaan in welke mate de gegenereerde code syntactisch correct is, voldoet aan stijlstandaarden, geen beveiligingsrisico's bevat, succesvol uitvoerbaar is, efficiënt presteert, schaalbaar is, onderhoudbaar is en functioneel correcte resultaten oplevert. Op basis van die resultaten wordt ook onderzocht welke controles, teststappen en beoordelingscriteria studenten kunnen gebruiken om onbetrouwbare output te detecteren en te verbeteren.
...
Concreet zoekt het onderzoek naar drie hoofdresultaten. Ten eerste een kwantitatief overzicht per model en per taak, waarin onder meer wordt gerapporteerd hoeveel gegenereerde scripts syntactisch correct zijn, voldoen aan stylestandaarden, geen beveiligingsproblemen bevatten, succesvol uitvoerbaar zijn, efficiënt presteren, onderhoudbaar zijn en functioneel correcte resultaten opleveren, zodat de studenten een geïnformeerde beslissing kunnen nemen wanneer zij een LLM kiezen om hun te helpen bij hun data engineering taken.
...
In Hoofdstuk~\ref{ch:resultaten} worden de resultaten van de evaluatie gepresenteerd en gevisualiseerd.
...
In Hoofdstuk~\ref{ch:wijzigingen} ten slotte worden de belangrijkste wijzigingen ten opzichte van de eerste versie opgelijst.